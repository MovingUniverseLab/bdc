...
Step 2) Extract starlist from these images with KAI. The /reduce directory has templates for the reduction and analysis steps.

Step 3) Run bdc_main.py to generate a distortion solution from the starlists.
...
#### instrument : 'OSIRSIS'
The instrument we are calculating a distortion model for. Currenly only handles 'OSIRIS'. Shouldn't be too hard to add other instruments in the future.
...
#### ref_instrument : 'PCU'	
The type of reference frame that we are aligning the observations to. Currently only handles 'PCU' and 'Hubble'.
...
The location of previous_distortion_model.
...
If True, Flystar will be used to propagate the positions of the stars in the reference frame forward to the observation epoch using the velocities in the reference list. If False, the propagation is performed in this script.
...
####  bad_files       : []
List any bad starlist filenames here to skip them.
...
####  target_DEC       : 0
####  single_fit      : True  
If True, will run Mosaic2Ref for each image individually. (Only works for True, should delete)
